...
# Parameters:
NUM_EXPERIENCES = 30	    # How many experiences stored per file
MIN_FILE_NUM = 10           # How many files at minimum do we need for training
PRE_STORED_DATA_FILES = 10   # How many files are already stored in our Data folder

MIN_FILES_IN_QUEUE = 10     # Files are added when this Number is reached
NEW_FILES_TO_ADD = 50      # How many files are added to the fifo file queue
...
class DataManager:
    ...
    def build_next_batch_op(self):
        ...
        features = tf.parse_single_example(serialized_experience, features={
            'state': tf.FixedLenFeature([40000], tf.float32),
            'action': tf.FixedLenFeature([2], tf.float32),
            'reward': tf.FixedLenFeature([1], tf.float32),
            'next_state': tf.FixedLenFeature([40000], tf.float32),
        ...
        # reshape gridmaps
        # print state.shape
        state = tf.reshape(state, [100, 100, 4])
        next_state = tf.reshape(next_state, [100, 100, 4])
